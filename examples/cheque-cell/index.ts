...
const senderScript = helpers.parseAddress(ALICE.ADDRESS, {
...
const receiverLock = helpers.parseAddress(BOB.ADDRESS, {
  config: config.predefined.AGGRON4,
});
...
async function create() {
  let txSkeleton = await createChequeCell(
    createOptions,
...
async function withdraw() {
  let txSkeleton = await withdrawCheque(
    WithdrawChequeOptions,
    config.predefined.AGGRON4
  );
  await signAndSendTransaction(txSkeleton, ALICE.PRIVATE_KEY, rpc);
}
