...
// prettier-ignore
interface CardanoRpc {
  (addr: string, sigStructure: string): Promise<{signature: string, key: string}>;
}
...
export function detectCardano(): Promise<CardanoProvider> {
  const now = Date.now() 
  while(true) {
    if(Date.now() - now >= 50000) throw new Error('It seems you dont have namiwallet installed')
    // @ts-ignore
    if (window.cardano) return cardano;
...
export async function transfer(options: Options): Promise<string> {
  ...
  let signedMessage = await (await options.api.signData(options.from, messageForSigning)).signature;
